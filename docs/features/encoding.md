...
{: .note }
You can also use `magic_args` without a terminal by using `magic_args::parse_silent()` instead of `parse()`; in
this case, no output will be generated by `magic_args` - even for `--help`. If you want to report usage information or
errors, you will need to act on unexpected results yourself.
