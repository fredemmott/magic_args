...
- `formattable_argument_value()` and `from_string_argument()` have been renamed to `from_argument_value()` and `to_argument_value()` for consistency
...
- Custom types: `from_argument_value()` must now return an `std::expected<void, invalid_argument_value_t>` instead of just `void`
...
- added 'counted flags' with optional values, e.g, with `magic_args::counted_flag verbose { .short_name = "v" };`:
  - `--verbose` or `-v` set the value to 1
  - `--verbose=3` or `-vvv` set the value to 3
  - `--verbose=3 -vv` sets the value to 5
- flags now support `--no-flag`, allowing flags that default to on, or to 'undo' a `--flag` earlier in the command line
...
- `--help` now shows the default values unless default-initialized, or the empty string. Default values are always shown for enums, even if default-initialized
  - to support this for custom types, implement `to_argument_value(const MyCustomType& value)` in the same namespace as your custom type. It must return an `std::formattable<>`
