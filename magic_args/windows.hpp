...
namespace magic_args::inline public_api {
  ...
  std::expected<T, incomplete_parse_reason_t> parse_silent(
    ...
    return parse_silent<T, Traits>(std::span{argvsv}, help);
  }
  ...
  std::expected<T, incomplete_parse_reason_t> parse_silent(
    ...
    return parse_silent<T, Traits>(buffer.c_str(), help);
  }
  ...
  std::expected<T, incomplete_parse_reason_t> parse(
    const wchar_t *const commandLine,
    const program_info &help = {},
    [[maybe_unused]] FILE *outputStream = stdout,
    [[maybe_unused]] FILE *errorStream = stderr) {
    return parse_silent<T, Traits>(commandLine, help);
  }
