...
[[nodiscard]]
constexpr std::ptrdiff_t last_positional_argument() {
  if constexpr (I == -1) {
    return -1;
  } else {
    using Traits = gnu_style_parsing_traits;
    constexpr auto behavior = argument_definition_t<T, I, Traits>::behavior;
    if constexpr (is_positional_argument(behavior)) {
      return I;
    } else {
      return last_positional_argument<T, I - 1>();
    }
  }
}
...
constexpr bool only_last_positional_argument_may_have_multiple_values() {
  ...
  if constexpr (I == N) {
    ...
    vector_like<typename argument_definition_t<T, I, Traits>::value_type>) {
    return I == last_positional_argument<T>();
  } else {
...
constexpr std::ptrdiff_t last_mandatory_positional_argument() {
  if constexpr (I == -1) {
    ...
    using Traits = gnu_style_parsing_traits;
    constexpr auto behavior = argument_definition_t<T, I, Traits>::behavior;
    if constexpr (behavior == Behavior::MandatoryPositionalArgument) {
      return I;
    } else {
...
constexpr std::ptrdiff_t first_optional_positional_argument() {
  if constexpr (I == N) {
    ...
    using Traits = gnu_style_parsing_traits;
    constexpr auto behavior = argument_definition_t<T, I, Traits>::behavior;
    if constexpr (is_positional_argument(behavior) && !is_required(behavior)) {
      return I;
    } else {
